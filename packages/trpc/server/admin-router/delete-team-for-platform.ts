...
export const deleteTeamForPlatformRoute = adminTokenProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    // every org member. For automated teardown this means one email per delete
    // to the platform-admin (the only owner-group member). Cost is low;
    // avoiding it would require modifying the upstream helper and carrying
    // the change through every upstream sync.
    await deleteTeam({
      userId: ctx.user.id,
      teamId: input.teamId,
    });
